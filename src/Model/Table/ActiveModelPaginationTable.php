...
class ActiveModelPaginationTable extends AdminTable
{
        ...
        foreach ($this->paginationReader->getData() as $row) {
            ...
            foreach ($this->paginationReader->model->getTypeList() as $type) {
                ...
                if ($type->visible->table) {
                    ...
                    if (!$row->getModelValue($this->model->active)) {
                    $parentCom = new Strike($tableRow);
                    }

                    /** @var AbstractModelItem $item */
                    $item = new $type->tableItemClassName($parentCom);
                    $item->row = $row;
                    $item->type = $type;
